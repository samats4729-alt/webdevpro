...
export function GreetingStep({ data, onChange, botName }: { data: { mode: 'ai' | 'template'; text: string; trigger?: string; keywords?: string[]; media?: string; ai_style?: string; ai_prompt?: string }; onChange: (updates: any) => void; botName?: string }) {
    ...
    const PROMPTS = {
        ...
        sales: `ДЕЙСТВУЙ КАК опытный менеджер по продажам ${botName || 'магазина'}.\nТВОЯ ЦЕЛЬ: Убедить клиента совершить покупку или записаться прямо сейчас.\nПРАВИЛА:\n1. Акцентируй внимание на выгодах и акциях.\n2. Используй триггеры срочности (только сегодня, осталось мало мест).\n3. Используй эмодзи 🔥❗️ для акцентов.\n4. Всегда закрывай диалог призывом к действию (CTA).\n5. Отрабатывай возражения мягко, но настойчиво.`
    };
    ...
    const handleGenerate = () => {
        ...
        setTimeout(() => {
            ...
            if (currentPrompt.includes('деловой') || currentPrompt.includes('вы')) {
                generatedText = `Здравствуйте! Вас приветствует ${botName || 'наш бот'}. Мы ценим ваше время. Чем можем быть полезны?`;
            } else if (currentPrompt.includes('дружелюбный') || currentPrompt.includes('ты') || currentPrompt.includes('😊')) {
                generatedText = `Привет! 👋 Я ${botName || 'твой помощник'}. Рад тебя видеть! Чем могу помочь? 😊`;
            } else if (currentPrompt.includes('продающий') || currentPrompt.includes('магазин') || currentPrompt.includes('🔥')) {
                generatedText = `🔥 Привет! Это ${botName || 'наш магазин'}. У нас для тебя супер-предложение! Хочешь узнать подробности?`;
            } else {
                generatedText = `Привет! Я ${botName || 'умный бот'}. Вижу твою инструкцию: "${aiPrompt.substring(0, 20)}...". Готов работать по ней!`;
            }
